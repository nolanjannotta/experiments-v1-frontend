...
async function getThumbnails(page, totalTokens, editionId) {
  ...
  if((page+1) * limit > totalTokens) {  
    ...
    try {
      const response = await fetch(`https://base-sepolia.g.alchemy.com/nft/v3/${process.env.NEXT_PUBLIC_BASE_SEPOLIA_URL}/getNFTsForContract?contractAddress=${artAddress}&withMetadata=true&startToken=${start}&limit=${limit}`, options)
